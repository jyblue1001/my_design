...
.control
  ...
  while run <= runs
    ...
    while code < 128
      ...
      save all
      op
      wrdata /foss/designs/my_design/projects/montecarlo/xschem_ngspice/dactut/mcdactut/mcdactut\{$&run\}.txt v(b0) v(b1) v(b2) v(b3) v(b4) v(b5) v(b6) v(Vout)/1e4 v(Vout)
      if code eq 0
        set appendwrite 
