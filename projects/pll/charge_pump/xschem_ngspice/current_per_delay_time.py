...
# File paths
intervals = np.arange(-12, 12, 1)
file_paths = [f'pfd_charge_pump_dead_zone_check_{i}.csv' for i in intervals]

# Extract delay times from file names
delay_times = [float(os.path.basename(path).split('_')[-1].split('.')[0]) for path in file_paths]

# Function to integrate current over time and calculate average current
def integrate_current(file_path):
    data = pd.read_csv(file_path, header=None, names=["time", "current"])
    time = data['time']
