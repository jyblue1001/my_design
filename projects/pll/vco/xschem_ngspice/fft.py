...
# Filepath of the text file
filepath = 'tb_current_starved_VCO2_110.csv'  # Replace with your actual filename

# Load the data from the text file
data = np.loadtxt(filepath)  # Assuming space-separated values
time = data[:, 0]  # Time values
original_data = data[:, 1]  # Original signal

# Add white noise to the original signal using AWGN
...
# Plot all in 3x2 subplots
fig, axs = plt.subplots(3, 2, figsize=(14, 12))

# Plot the original data (top left)
axs[0, 0].plot(time, original_data)
axs[0, 0].set_title(f'Time Domain View of Original Data - {filepath}')
